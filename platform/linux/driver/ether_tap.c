...
ether_tap_open(struct net_device *dev)
{
    ...
    ifr.ifr_flags = IFF_TAP | IFF_NO_PI; // TAPを作成するようにフラグを設定 (L2: TAP, L3: TUN)
    if (ioctl(tap->fd, TUNSETIFF, &ifr) == -1) { // TAPを作成
        errorf("ioctl(TUNSETIFF): %s, dev=%s", strerror(errno), dev->name);
        close(tap->fd);
        return -1;
    }
    if (fcntl(tap->fd, F_SETOWN, getpid()) == -1) { // 所有者を自分に設定
        errorf("fcntl(F_SETOWN): %s, dev=%s", strerror(errno), dev->name);
        close(tap->fd);
        return -1;
    }
    val = fcntl(tap->fd, F_GETFL, 0); // フラグを取得
    if (val == -1) {
        errorf("fcntl(F_GETFL): %s, dev=%s", strerror(errno), dev->name);
        close(tap->fd);
        return -1;
    }
    if (fcntl(tap->fd, F_SETFL, val | O_ASYNC | O_NONBLOCK) == -1) { // フラグをセット
...
ether_tap_input(struct net_device *dev, uint8_t *frame, size_t flen)
{
    ...
    if (flen < (ssize_t)sizeof(*hdr)) { // 最低限 イーサネットヘッダがないと困る
        errorf("too short");
        return -1;
    }
    hdr = (struct ether_hdr*)frame;
